...
var rValue = sessionStorage.getItem("r-value") == null ? sessionStorage.setItem("r-value", "1.0") : sessionStorage.getItem("r-value");

function updateR(r) {
    sessionStorage.setItem("r-value", r);
...
function drawPoint(x, y, res) {
    ...
    ctx.beginPath();
    if (res) {
        ctx.fillStyle = 'rgb(85,117,85)';
        ctx.strokeStyle = 'rgb(85,117,85)';
    } else {
        ctx.fillStyle = 'rgb(255,0,47)';
        ctx.strokeStyle = 'rgb(255,0,47)';
    }
    ctx.moveTo(x, y);
    ...
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = 'rgb(252,29,99)';
    ctx.strokeStyle = 'rgb(33,32,32)';
...
function drawAxisLabel(context, label, x, y) {
    context.fillStyle = 'rgb(50,50,66)'; // Цвет и прозрачность заливки
...
function shadeRegion(context, centerX, centerY, r) {
    context.fillStyle = 'rgb(252,29,99)'; // Цвет и прозрачность заливки

    //part of circle
    context.beginPath();
    context.arc(centerX, centerY, r / 2, Math.PI / 2, Math.PI, false);
